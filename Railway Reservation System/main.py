...
def book_ticket(train_number, passenger_name, passenger_age, passenger_gender, seat_type):
    ...
    if train_data:
        ...
        if seat_number:
            ...
            # 1. Calculate Total Amount (replace with your logic)
            amount_to_charge = 1000  # Example: Rs 1000 

            # 2. Payment Confirmation from User (Streamlit UI)
            if st.button("Confirm Payment"): 
                try:
                    # 3. Process Payment (Example: Stripe)
                    ...
                    if payment_intent['status'] == 'succeeded':
                        ...
                        c.execute(f"UPDATE seats_{train_number} SET booked=1, seat_type=?, passenger_name=?, passenger_age=?, passenger_gender=?, seat_number=? WHERE seat_number=?", 
                                    (seat_type, passenger_name, passenger_age, passenger_gender, seat_number[0], seat_number[0]))
                        conn.commit()
